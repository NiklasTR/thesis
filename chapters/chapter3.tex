...
The basic goal of matrix factorization methods is to identify a small number of factors in measured data that explain its intrinsic structure. In this process, a data matrix \( Y \), consisting of \( n \) samples and \( m \) features, is decomposed into two lower-dimensional matrices: 
...
\begin{enumerate}
    ...
    \item The factor weight matrix \( W^T \), which has dimensions \( k \times m \). Here, each row corresponds to a latent factor, and each column corresponds to a feature. 
\end{enumerate}

The factor score matrix \( Z \) is also referred to as a learnt, lower-dimensional representation of the data. The choice of \( k \) (the number of latent factors) ideally strikes a balance between simplicity and comprehensiveness and can be algorithmically determined.  

In group factor analysis, including multi-omics factor analysis (MOFA), the matrix factorization method is extended to multiple data modalities: A set of \(m\) data matrices \( Y^m \) is decomposed into a single factor score matrix \( Z \) as well as a corresponding set of  \(m\) weight matrices \( W^T^m \). Factors that are identified using this approach describe axes of variation within the data that can span multiple different data modalities. Next to simplifying the interpretation of complex data, factors that have been learnt to represent the structure of measurements across multiple modalities with group factor analysis enable more reliable similarity measurements and missing data prediction than alternative factor learning methods or raw features data \citep{Klami}.
...
% The same way that more similar compounds can be retrieved using factors than raw chemical descriptors, the more compouds that are similar to APC mutant organoids can be retrieved using factors over raw image descriptors
